...
const TargetAudienceSection: React.FC = () => {
  ...
  return (
    <section className="relative w-full py-24 md:py-32 bg-[#0d0d0d] text-white overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          <span className="inline-block px-4 py-1 mb-6 border border-brand-accent/30 rounded-full bg-brand-accent/5 text-brand-accent text-xs font-bold uppercase tracking-widest">
            Identificação
          </span>
        ...
        <div className="flex flex-wrap justify-center gap-6 md:gap-8">
          {audiencePoints.map((item, index) => (
            <div 
              key={index}
              className="w-full md:w-[calc(50%-16px)] lg:w-[calc(33.333%-22px)] group relative"
            >
              <div className="h-full p-8 md:p-10 rounded-2xl bg-[#151515] border border-white/5 hover:border-brand-accent/40 transition-all duration-300 hover:bg-[#1a1a1a] hover:-translate-y-2 hover:shadow-[0_10px_30px_-10px_rgba(212,175,55,0.15)] flex flex-col items-start text-left">
                
                {/* Header of Card */}
                <div className="flex justify-between items-start w-full mb-6">
                  <div className="p-4 rounded-lg bg-brand-accent/10 text-brand-accent group-hover:bg-brand-accent group-hover:text-black transition-colors duration-300">
                    {item.icon}
                  </div>
